...
def Q_Calculate( N , Service_Rate , Live_Time):
    ...
    Farctorial = math.factorial( N );

    Mult = 1;

    for j in range(0 , N + 1):

        Mult = Mult * (Service_Rate + (j / Live_Time))
...
while Entry_Rate < 20.1:
    ...
    with open('Drops(Exp).txt','a') as Output:
    ...
    with open('Blocks(Exp).txt','a') as Output:
    ...
    with open('Customers_In_Queque(Exp).txt','a') as Output:
